...
public class FpDayDietController {
	...
	@GetMapping("/one")
	public ModelAndView selectOnedaydiet(ModelAndView mv, String mid) {
		mv.addObject("daydietone", fpdaydietService.selectOne(mid));
	...
	@GetMapping("/update")
	public ModelAndView updatedaydiet(ModelAndView mv, String mid ) {
		mv.addObject("daydietone", fpdaydietService.selectOne(mid));
	...
	public String deleteDoMemeber(RedirectAttributes redirectAttr, String mid ) {
		String viewPage = "redirect:/";
		int result = fpdaydietService.delete(mid);
		try {
			if (result < 1) {
